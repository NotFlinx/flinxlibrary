...
local FlinxFolder = "Flinx"
local ConfigurationFolder = FlinxFolder.."/Configurations"
...
local FlinxLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Flinx
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Flinx
...
local Flinx = game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn.protect_gui then 
	...
	Flinx.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Flinx.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Flinx.Parent = CoreGui
end
...
local Main = Flinx.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Flinx.DisplayOrder = 100
...
local Notifications = Flinx.Notifications

local SelectedTheme = FlinxLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = FlinxLibrary.Theme[ThemeName]
	for _, obj in ipairs(Flinx:GetDescendants()) do
	...
	Flinx.Main.BackgroundColor3 = SelectedTheme.Background
	Flinx.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Flinx.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Flinx.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Flinx.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Flinx.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Flinx.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if FlinxLibrary.Flags[FlagName] then
			spawn(function() 
				if FlinxLibrary.Flags[FlagName].Type == "ColorPicker" then
					FlinxLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if FlinxLibrary.Flags[FlagName].CurrentValue or FlinxLibrary.Flags[FlagName].CurrentKeybind or FlinxLibrary.Flags[FlagName].CurrentOption or FlinxLibrary.Flags[FlagName].Color ~= FlagValue then FlinxLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			FlinxLibrary:Notify({Title = "Flag Error", Content = "Flinx was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(FlinxLibrary.Flags) do
...
function FlinxLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= FlinxLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Flinx | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	FlinxLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function FlinxLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Flinx Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Flinx Interface Suite" then
		LoadingFrame.Version.Text = "Flinx UI"
	end
	...
	pcall(function()
		...
		if not isfolder(FlinxFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(FlinxFolder.."/Discord Invites") then
			makefolder(FlinxFolder.."/Discord Invites")
		end
		if not isfile(FlinxFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(FlinxFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Flinx RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(FlinxFolder.."/Key System") then
			makefolder(FlinxFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Flinx | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(FlinxFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(FlinxFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(FlinxFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						FlinxLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Flinx | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					FlinxLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Flinx | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Flinx | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					FlinxLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Flinx | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					FlinxLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Flinx | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					FlinxLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Flinx | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					FlinxLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function FlinxLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				FlinxLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, FlinxLibrary.LoadConfiguration, FlinxLibrary)
